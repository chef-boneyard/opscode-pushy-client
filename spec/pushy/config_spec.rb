...
describe PushyClient::Client do
  ...
  let(:setup_config_service) { PushyClient::Client.service_url_base = config_service }
  ...
  pending '.load!' do
    subject { given; PushyClient::Client.load! }
  ...
  context 'with configuration endpoint' do
    ...
    let(:pushy_client) { given; PushyClient::Client.load! }
    ...
    its(:node_name)   { given; ap [PushyClient::Client.service_url_base];  should eql host }
    end
